<script lang="ts">
  ...
  async function calculatePages() {
    ...
    if (resumeContent) {
      ...
      // Approximate height of a page in pixels.
      const pageHeight = 1050;
      pages = Math.ceil(contentHeight / pageHeight);
    }
  }

  onMount(calculatePages);
  afterUpdate(calculatePages);
</script>
  ...
  <div class="overflow-auto max-h-[800px] bg-gray-100 p-4">
    <div id="resume-preview-container">
      {#each Array(pages) as _, i}
        <ResumePage>
          <div class="resume-content-wrapper" style="top: -{i * 1050}px;" bind:this={resumeContent}>
                  ...
                  <div>
                    ...
                    <div class="flex flex-wrap gap-2">
                      {#each resumeData.skills as skill}
                        {#if skill.name}
                          <div class="bg-blue-50 border border-blue-200 rounded px-3 py-1 break-inside-avoid">
                ...
                {#if resumeData.skills.some(skill => skill.name)}
                  <div>
                    ...
                    <div class="flex flex-wrap gap-2">
                      {#each resumeData.skills as skill}
                        {#if skill.name}
                          <span class="bg-gray-100 px-3 py-1 rounded break-inside-avoid">{skill.name}</span>
                        {/if}
                      {/each}
                    </div>
                  </div>
